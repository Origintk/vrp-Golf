...
Citizen.CreateThread(function()
	...
	while true do
        ...
        if distance < 1.5 then
            ...
            if golfplaying then
                DisplayHelpText("Pressione ~g~~INPUT_CONTEXT~~s~ para terminar o jogo.")
            else
                DisplayHelpText("Pressione ~g~~INPUT_CONTEXT~~s~ para jogar golf ($100).")
            end
                ...
                if (golfplaying) then
                    ...
                    GFServer.payGolf({}, function(paid)
                      if paid then
                        ...
                        TriggerEvent("customNotification","Pressione E para rebater, A-D para rodar, Y para trocar o taco.")
                      else
                        TriggerEvent('chatMessage', "SERVER", {255, 0, 0}, "Você não tem dinheiro suficiente.")
                      end
                    end)
                end
            end
...
function dropShot()
	...
	while doingdrop do
		...
		if distance < 50.0 and distanceHole > 50.0 then
			DisplayHelpText("Pressione ~g~E~s~ para dropar aqui.")
			if ( IsControlJustReleased(1, 38) ) then
				doingdrop = false
			...
			DisplayHelpText("Pressione ~g~E~s~ para dropar - ~r~ muito longe da bola ou perto do buraco.")
		end
	end
end
